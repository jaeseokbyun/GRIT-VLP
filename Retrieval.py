...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i,(image, text, idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_itc, loss_itm = model(image, text_input,alpha=alpha, idx=idx)                  
...
def main(args, config):
    ...
    if args.checkpoint:    
        ...
        if config['use_momentum']:
             # Initialize it with initial network (initialized from BERT, ViT) or the same pre-trained network
             m_pos_embed_reshaped = interpolate_pos_embed(state_dict['visual_encoder.pos_embed'],model.visual_encoder)   
        ...
        # initialize the momentum model with the same pre-trained network
        # If use_momentum, there are a lot of missing keys since we don't have pre-trained momentum network.
        if args.copy_init:
            model.copy_params()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--copy_init', default=False, type=bool)
